 ...
 void OnGUI() 
	{
		...
		if (GUI.Button(new Rect(105, 60, 70, 30),"Death")){
			frog.GetComponent<Animation>().wrapMode= WrapMode.Loop;
			frog.GetComponent<Animation>().CrossFade("BG_Death");
		}

		if (GUI.Button(new Rect(180, 60, 70, 30),"GangnamStyle")){
			frog.GetComponent<Animation>().wrapMode= WrapMode.Loop;
